...
export default function NotificationSettings({csrfToken}){
 ...
 return <section id="notifications" className="content-block notification-settings"><div className="account-card"><span className="account-avatar"><Bell size={24}/></span><div><h2>Notifications</h2><p>Get notified when today’s Lunch Special is announced.</p></div></div>
  {state.error?<p className="form-status" role="alert">{state.error}</p>:null}
  {unavailable?<p>Lunch Special notifications are not available yet.</p>:support.needsInstall?<p>On iPhone or iPad, add Ladel’s to your Home Screen, then open it there to enable notifications.</p>:!support.supported?<p>This browser doesn’t support Lunch Special notifications.</p>:support.permission==="denied"?<p>Notifications are blocked in your browser settings. You can allow them there and return to this page.</p>:<>
   <div className="notification-state"><strong>{enabled?"Lunch Special notifications are on":"Lunch Special notifications are off"}</strong><span>{state.currentDevice?"This device is enabled":"This device is not enabled"} · {state.status.active_device_count} active {state.status.active_device_count===1?"device":"devices"}</span></div>
   {(!enabled||!state.currentDevice)?<button className="primary-button" disabled={state.busy} onClick={enable} type="button"><Bell size={17}/> {state.busy?"Enabling…":enabled?"Enable on this device":"Enable Lunch Special notifications"}</button>:null}
