...
export default function OrdersPageMobile() {
  ...
  return <section className="page-section ordering-page app-simple-page">
    ...
    {detail ? <div className="content-block app-content-block"><h2>Order {detail.public_token.slice(0, 8).toUpperCase()}</h2><p>{detail.items.map((item) => `${item.quantity} × ${item.product_name}`).join(", ")}</p><strong>{money(detail.total_cents)}</strong><div className="form-actions"><button className="primary-button" type="button" onClick={reorder}>Reorder</button><button className="secondary-button" type="button" onClick={() => setDetail(null)}>Back</button></div></div> : null}
