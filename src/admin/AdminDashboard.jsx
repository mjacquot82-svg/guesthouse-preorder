...
export default function AdminDashboard() {
  ...
  const cloverDisplay = (() => {
    ...
    if (clover.status === "ready" && clover.connected) return { detail: [clover.environment ? `Environment: ${clover.environment}` : null, clover.merchant_id ? `Merchant: ${clover.merchant_id}` : null].filter(Boolean).join(" · ") || "Clover is ready.", heading: "Connected" };
  ...
  return <section className="page-section operations-dashboard">
    ...
    <div className="dashboard-grid">
      {canReadOrders ? <>
        <Link className="metric-card metric-card-link" to="/admin/orders"><span>Orders waiting</span><strong>{orderSummary.status === "loading" ? "—" : orderSummary.status === "ready" ? orderSummary.new : "Unavailable"}</strong><p>{orderSummary.status === "ready" ? `${orderSummary.preparing} preparing · ${orderSummary.ready} ready` : orderSummary.status === "loading" ? "Loading today’s queue…" : "Orders could not be loaded."}</p></Link>
      ...
      {canReadCatalog ? <Link className="metric-card metric-card-link" to="/admin/products"><span>Sold-out products</span><strong>{soldOut}</strong><p>{soldOut ? "Review today’s unavailable items" : "Everything published is available"}</p></Link> : null}
