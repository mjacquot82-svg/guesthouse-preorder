  ...
  return <section className="page-section admin-products-page">
    ...
    <div className={`admin-products-layout${canEdit ? "" : " availability-only"}`}>
      ...
      {canEdit ? <section className="product-editor-panel" aria-labelledby="product-editor-heading"><div className="section-heading"><div><p className="eyebrow">Quick edit</p><h2 id="product-editor-heading">{selectedProduct ? `Edit ${selectedProduct.name}` : "Add a product"}</h2></div></div><form className="product-form" aria-busy={saving} onSubmit={handleSubmit}>
        ...
        <details className="product-advanced" open={Boolean(selectedProduct)}><summary>Customer options</summary>{modifierGroups.some((group) => group.active) ? <fieldset className="admin-modifier-picker"><legend>Which options can customers choose for this product?</legend>{modifierGroups.filter((group) => group.active || formProduct.modifierGroupIds.includes(group.id)).map((group) => { const assigned = formProduct.modifierGroupIds.includes(group.id); return <label key={group.id}><input checked={assigned} disabled={!group.active && !assigned} type="checkbox" onChange={() => toggleModifierGroup(group.id)} /><span><strong>{group.name}</strong><small>{group.selectionType === "single" ? group.required ? "Must choose one" : "Can skip" : group.maxSelections ? `Can choose up to ${group.maxSelections}` : "Can choose more than one"} · {group.options.filter((item) => item.active).length} choices{group.active ? "" : " · Unavailable"}</small></span></label>; })}</fieldset> : <div className="modifier-assignment-empty"><strong>No customer options yet.</strong><p>Create a customer option group to let customers customize this product.</p></div>}<button className="secondary-button" type="button" onClick={() => setManagingModifiers(true)}>Manage customer options</button></details>
